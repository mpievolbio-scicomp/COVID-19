...
for t=ts+dt:dt:ts+tmstep
    ...
    dt1=dt;
    %first step
    ESenter=dt1*(ones(num_loc,1)*theta).*(M(:,:,ts)*(S(:,:,tcnt)./(pop-Is(:,:,tcnt))));
    ESleft=min(dt1*(ones(num_loc,1)*theta).*(S(:,:,tcnt)./(pop-Is(:,:,tcnt))).*(sum(M(:,:,ts))'*ones(1,num_ens)),dt1*S(:,:,tcnt));
    EEenter=dt1*(ones(num_loc,1)*theta).*(M(:,:,ts)*(E(:,:,tcnt)./(pop-Is(:,:,tcnt))));
    EEleft=min(dt1*(ones(num_loc,1)*theta).*(E(:,:,tcnt)./(pop-Is(:,:,tcnt))).*(sum(M(:,:,ts))'*ones(1,num_ens)),dt1*E(:,:,tcnt));
